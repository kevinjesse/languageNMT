'''
Implement Sequence to Sequence Neural Model from the Paper: Neural Machine Translation by Jointly Leanring to Align and Translate
'''
...
#Network Structured
PARSER.add_argument('--embedding_size',type=int,default=128,help='embedding layer size for both encoder and decoder')
PARSER.add_argument('--hidden_size',type=int,default=256,help='hidden state size for both encoder and decoder')
...
PARSER.add_argument('--n_epochs', type=int, default=50, help='maximum number of epochs to run')
...
if BPE_dataset_suffix is not None:
    ...
    val_ori_data = val_data
    test_ori_data = test_data
...
baseline_model = NMT_Seq2Seq(input_size,\
                             output_size,\
                             embedding_size,\
                             embedding_size,\
                             hidden_size,\
...
#############################################################3. Print out the Network and Configuration Setting#################################################################################################
...
#############################################################4. Start the Traning Process#################################################################################################
#Initialize some parameters for this training process
iter_count = 0
best_bleu = 0
best_meteor = 0
best_loss = 1000
...
for epoch in range(1,n_epochs + 1):
    for batch_x,batch_y,batch_x_lengths,batch_y_lengths in data_generator_tl(train_data_index,batch_size):
        ...
        loss = train_nmt(input_variable, target_variable, batch_x_lengths,baseline_model,criterion,optimizer,teacher_force_ratio=teacher_forcing_ratio,clip=clip)
        ...
        if iter_count%eval_every == 0:
            ...
            if val_loss_mean < best_loss:
                torch.save(baseline_model,os.path.join(trained_model_output_path,'seq2seq_model_best_loss.pt'))
                #update the best_loss
                best_loss = val_loss_mean

            if val_bleu[0] > best_bleu:
                torch.save(baseline_model,os.path.join(trained_model_output_path,'seq2seq_model_best_BLEU.pt'))
            ...
            if val_meteor[0] > best_meteor:
                torch.save(baseline_model,os.path.join(trained_model_output_path,'seq2seq_model_best_METEOR.pt'))
                #update the best_bleu score
        ...
        if iter_count%save_every == 0:
            ...
            torch.save(baseline_model,os.path.join(trained_model_output_path,'seq2seq_model_{}.pt'.format(iter_count)))
        if early_stop == 0:
            break
        iter_count += 1
    if early_stop==0:
...
#############################################################4. Evaluate the Best Saved Model on Test Dataset#################################################################################################
#Save the Model
print("Evalute the Test Result")

#########################Use the best BLEU Model to Evaluate#####################################################
#Load the Best BLEU Model
best_model = torch.load(os.path.join(trained_model_output_path,'seq2seq_model_best_BLEU.pt'))
if use_cuda:
    best_model.cuda()
...
best_loss_model = torch.load(os.path.join(trained_model_output_path,'seq2seq_model_best_loss.pt'))
if use_cuda:
